...
class DiscreteTimeGaussianDiffusion(base.GaussianDiffusion):
    ...
    def q_sample(self, x0, steps, noise):
        ...
        return xt, alpha_bar.sqrt() * x0, (1 - alpha_bar).sqrt()
    ...
    def p_sample(
        ...
        prediction = self.denoiser(x_t, steps, text_features)

        if(self.classifier_free_scale is not None):
            prediction_null = self.denoiser(x_t, steps, text_null_features)
            # classifier-free: e_guide = e_null + s · (e_condition - e_null)
            prediction = prediction_null + self.classifier_free_scale * (prediction - prediction_null)
    ...
    def sample(
        ...
        return noise, x if return_noise else x

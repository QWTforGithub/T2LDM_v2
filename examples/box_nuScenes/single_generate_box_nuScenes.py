...
from examples.box_nuScenes.config_box_nuScenes_gn import TrainingConfig

import examples.box_nuScenes.loading_box_nuScenes  as inference_mgpus
...
def main(args, cfg):
    ...
    if(cfg.use_text or cfg.use_camera or cfg.use_control_net):
        ...
        points = common.get_lidar_sweep(args.reference_file_path, return_intensity=True, return_time=True, dim=5)
        with open(file=args.box_file_path, mode="rb") as f:
            box = pickle.load(f)
            lidar_semantic = []
            lidar_semantic.append(box["gt_boxes"])
            lidar_semantic.append(box["gt_names"])

            gt_class = lidar_semantic[0][:, -1]
            corners = common.boxes_to_corners_3d(lidar_semantic[0])
            semantic = common.get_semantic_from_corners(
                points[:, :3],
                corners,
                gt_class,
                background_id=cfg.semantic_class_num
            )
        ...
        semantic = range_image[[5]] / cfg.semantic_class_num
...
if __name__ == "__main__":
    ...
    ckpt = "/root/models/checkpoints/nuScenes_3DBox_0000400000.pth"
    ...
    # num_steps = 400_000
    random_num = 256
    project_dir = "examples/box_nuScenes"
    project_dir = f"{root_path}/{project_dir}/test_box_nuScenes"
    load_config = False

    box_file_path = "../example_files/nuScenes_box.pkl"
    reference_file_path = "../example_files/nuScenes_lidar.bin"
    ...
    parser.add_argument("--box_file_path", type=str, default=box_file_path)
    parser.add_argument("--reference_file_path", type=str, default=reference_file_path)

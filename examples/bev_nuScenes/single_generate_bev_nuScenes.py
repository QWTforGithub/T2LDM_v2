...
from examples.bev_nuScenes.config_bev_nuScenes_gn import TrainingConfig

import examples.bev_nuScenes.loading_bev_nuScenes  as inference_mgpus
...
def main(args, cfg):
    ...
    if(cfg.use_text or cfg.use_camera or cfg.use_control_net):
        ...
        bev = cv2.imread(args.bev_file_path, cv2.IMREAD_GRAYSCALE).astype(np.float32)
        bev = bev / 255.0

        bev = np.expand_dims(bev, axis=0)
        bev = np.repeat(bev, 3, axis=0)
...
if __name__ == "__main__":
    ...
    # num_steps = 400_000
    random_num = 256
    project_dir = "examples/bev_nuScenes"
    project_dir = f"{root_path}/{project_dir}/test_bev_nuScenes"
    load_config = False

    bev_file_path = "../example_files/nuScenes_bev.png"
    ...
    parser.add_argument("--bev_file_path", type=str, default=bev_file_path)

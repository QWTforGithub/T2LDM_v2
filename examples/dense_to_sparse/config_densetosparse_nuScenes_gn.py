...
'''
        ...
        For example, if you want to conduct the downsampling 4x:
            ...
            base_up_rate: float = 0.25 # This is the GT. This will get the GT from LIDAR_TOP_DOWNSAMPLING{base_up_rate}.
            downsampling: bool = True
            down_rate: float = 0.25 # This will implement the downsampling 4x than the original resolution.
            base_down_rate: float = 1.0 # The is the condition. This will get the condition from LIDAR_TOP_DOWNSAMPLING{base_up_rate}.
            interpolate_rate: float = -1 # if you want to use the interpolate guidance, please set the interpolate rate.

        Similarly, if you want to conduct the upsampling 4x:    
            resolution: tuple[int, int] = (32, 1024)
            upsampling: bool = True 
            up_rate: float = 4 # This will implement the upsampling 4x than the original resolution x 0.25.
            base_up_rate: float = 1.0 # This is the GT. This will get the GT from LIDAR_TOP_DOWNSAMPLING{base_up_rate}.
        ...
        And, you also can conduct the upsampling based on the original resolution (32, 1024):
            ...
            base_up_rate: float = 2.0 # This is the GT. This will get the GT from LIDAR_TOP_DOWNSAMPLING{base_up_rate}.
            base_down_rate: float = 1.0 # The is the condition. This will get the condition from LIDAR_TOP_DOWNSAMPLING{base_up_rate}.
            interpolate_rate: float = -1 # if you want to use the interpolate guidance, please set the interpolate rate.

        The key approach is to set 'base_up_rate (the GT)' and 'base_down_rate (the condition)'.
'''
...
class TrainingConfig:
    ...
    resolution: tuple[int, int] = (32, 256)  # if you want to condut S2D or S2D, please set the resolution.
    depth_range: tuple[int, int] = (0.01, 50.0)
    fov: tuple[int, int] = (3, -25)
    batch_size_train: int = 32  # 2 # 16
    batch_size_eval: int = 4
    num_workers: int = 32  # 2 # 16

    text_keys: str = "text"  # "text_l0 text_l1 text_2"
    pkl: str = "nuscenes.pkl"
    ...
    n_attn_types = ["linear", "linear", "linear",
                    "vanilla"]  # ["nn", "nn", "nn", "nn"] # ["linear", "linear", "linear", "vanilla"]
    ...
    upsampling: bool = False  # False # True
    up_rate: float = 4  # The upsampling resolution = base_down_rate * up_rate
    base_up_rate: float = 1.0  # The param indicates the dataset name as the GT. LIDAR_TOP_DOWNSAMPLING{base_up_rate}
    downsampling: bool = True  # False
    down_rate: float = 0.25  # The downsampling resolution = base_up_rate * down_rate
    base_down_rate: float = 1.0  # The param indicates the dataset name as the condition. LIDAR_TOP_DOWNSAMPLING{base_up_rate}
    interpolate_rate: float = 0.5  # if you want to use the interpolate guidance, please set the interpolate rate.
    # ---- model setting ----
    ...
    pretrained_checkpoint_dir: str = "/root/models/IJCV/nuScenes/unconditional/diffusion_0000400000.pth"
    # ---- training setting ----
